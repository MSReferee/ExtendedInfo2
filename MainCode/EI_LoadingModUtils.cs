...
namespace IINS.ExtendedInfo
{
    ...
    /// </summary>
    internal static class EIModUtils
    {
        ...
        internal static string GetAssemblyPath()
        {
            ...
            foreach (PluginManager.PluginInfo plugin in plugins)
            {
                try
                {
                    ...
                    if (mods.FirstOrDefault() is ExtendedInfoMod)
                    {
            ...
            throw new Exception("Failed to find Extended Info assembly!");
            //throw new FileNotFoundException(DistrictStatsMod.ModName + ": assembly path not found!");
        }
